...
| `field_type_overrides` | A mapping from fully qualified Protobuf field names (i.e. a fully qualified Protobuf message name followed by a dot "." followed by the field name) to fully qualified ROS message type names. Unlike `message_mapping`, which remaps whole Protobuf message types, this overrides the ROS type of a single field, leaving the Protobuf definition untouched. A repeated field may only be overridden with a fixed-size array of primitive types (e.g. `float64[36]`). | `{}` |
